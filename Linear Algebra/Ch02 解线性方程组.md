...
#TODO
## 习题
> [!PDF|yellow] [[Introduction to Linear Algebra.pdf#page=65&selection=863,0,902,1&color=yellow|Introduction to Linear Algebra, p.55]]
> > Could a 4 by 4 matrix A be invertible if every row contains the numbers 0,1, 2,3 in some order?
> 
> 对角线为0: 四个列向量位于四个空间，线性无关，秩为4，可逆
> (事实上，没有0作为列向量即可)

> [!PDF|yellow] [[Introduction to Linear Algebra.pdf#page=65&selection=903,1,931,1&color=yellow|Introduction to Linear Algebra, p.55]]
> > What if every row of B contains 0, 1, 2, —3 in some order 
> 
> 和为0，Ax=0，x=(1,1,1,1)满足

> [!PDF|yellow] [[Introduction to Linear Algebra.pdf#page=66&selection=478,0,504,2&color=yellow|Introduction to Linear Algebra, p.56]]
> > Show that P — Q is singular by solving (P — Q)x = 0.
> 
> $x=\begin{bmatrix}1 & 1 & 1 & 1 & \dots & 1\end{bmatrix}^T$ 经过行交换不变，故 (P-Q)x=0，P-Q不可逆
> 
> **奇异矩阵** (singular matrix) 是指一个**行列式为零**的方阵
